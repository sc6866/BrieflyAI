...
import { generateBriefing, generateAudioBriefing } from './services/geminiService';
...
const EMAIL_TEMPLATE_CODE = `
<div style="font-family: -apple-system, system-ui, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
  <div style="background-color: #0f172a; padding: 32px 24px; text-align: center;">
    <h1 style="margin: 0; color: #ffffff; font-size: 22px; font-weight: 800; letter-spacing: 0.5px;">BRIEFLY AI · 生产级情报内参</h1>
    <p style="margin: 8px 0 0; color: #94a3b8; font-size: 13px;">{{date}} | 自动决策引擎</p>
  </div>
  ...
  <div style="background-color: #f8fafc; padding: 20px; text-align: center; color: #94a3b8; font-size: 11px; border-top: 1px solid #e2e8f0;">
    <p style="margin: 0;">本内参由 BrieflyAI 生产环境自动推送</p>
    <p style="margin: 4px 0 0;">旨在提供精准、降噪的全球商业情报</p>
  </div>
</div>
`;
...
const App: React.FC = () => {
  ...
  const generateEmailHtml = (data: BriefingReport) => {
    ...
    html += `
      <div style="background-color: #0f172a; border-radius: 10px; padding: 20px; color: #ffffff;">
        <h3 style="margin: 0 0 18px; color: #38bdf8; font-size: 14px; text-align: center; font-weight: 800; text-transform: uppercase;">🔥 流量信号雷达 (Douyin/TikTok)</h3>
        ${data.trending.slice(0, 5).map(t => `
          <div style="margin-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 12px;">
            <p style="margin: 0 0 6px; font-weight: 700; font-size: 13px;">${cleanProductionText(t.topic)} <span style="font-size: 10px; color: #38bdf8; margin-left: 8px;">(${t.heat})</span></p>
            <div style="background: rgba(255,255,255,0.03); padding: 8px; border-radius: 4px; font-size: 11px; color: #cbd5e1; border-left: 2px solid #38bdf8;">
              ${cleanProductionText(t.analysis)}
            </div>
          </div>
        `).join('')}
      </div>
  ...
  const handleSync = async (silent = false) => {
    ...
    try {
      ...
      if (prefs.barkKey) safePush(`https://api.day.app/${prefs.barkKey}/${encodeURIComponent(reportTitle)}/${encodeURIComponent(mobileBody)}?group=BrieflyAI&icon=https://cdn-icons-png.flaticon.com/512/3665/3665964.png`);
      ...
      if (!silent) alert("情报同步中断: " + err.message);
    }
  };
                ...
                {activeTab === 'brief' ? (
                  ...
                  <div className="space-y-12 py-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4 py-6 bg-[var(--input-bg)]/40 rounded-[2.5rem] border border-[var(--border)]">
                       ...
                       <button onClick={() => handleSync()} disabled={status === ReportStatus.FETCHING} className="flex items-center justify-center gap-3 px-8 py-3 bg-[var(--accent)] text-white rounded-2xl shadow-xl shadow-[var(--accent)]/30 hover:scale-105 active:scale-95 transition-all">
                         <svg className={`w-5 h-5 ${status === ReportStatus.FETCHING ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeWidth="3" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                         <span className="text-xs font-black uppercase tracking-widest">{status === ReportStatus.FETCHING ? '刷新中...' : '重载全网脉冲'}</span>
                       </button>
                    </div>
          ...
          <div className="bg-[var(--card-bg)] w-full max-w-xl rounded-[3rem] p-10 border border-[var(--border)] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.5)] overflow-y-auto max-h-[90vh] no-scrollbar space-y-10" onClick={e => e.stopPropagation()}>
             ...
             <div className="space-y-8">
                ...
                <div className="space-y-4">
                   <h4 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">生产环境 HTML 模板</h4>
                   <div className="relative group">
                      <pre className="bg-black text-[#00ff88] p-5 rounded-[1.5rem] text-[10px] font-mono overflow-x-auto h-40 no-scrollbar border border-white/5">{EMAIL_TEMPLATE_CODE}</pre>
                      <button onClick={() => { navigator.clipboard.writeText(EMAIL_TEMPLATE_CODE); alert("生产级模板已复制。"); }} className="absolute top-3 right-3 bg-white/10 backdrop-blur-md hover:bg-white/20 px-3 py-1.5 rounded-xl text-[9px] font-bold transition-all">复制核心代码</button>
                   </div>
                </div>
